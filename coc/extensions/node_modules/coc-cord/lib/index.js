...
const getPresence = (startTimeStamp) => {
    let presence = {
        startTimestamp: startTimeStamp,
        smallImageKey: "nvim",
        smallImageText: "NeoVim text editor"
    };
    let idlingPresence = { largeImageKey: "nvim", largeImageText: "NeoVim text editor", state: undefined, details: "idling" };
    presence = { ...presence, ...idlingPresence };
    let doc = coc_nvim_1.workspace.getDocument(coc_nvim_1.workspace.uri);
    if (!doc) {
        return presence;
    }
    ...
    presence.state = `Workspace: ${coc_nvim_1.workspace.root.split("/").pop()}`;
    ...
    const getFileName = (lang) => (lang.extension === "" ? lang.fileName : lang.fileName + "." + lang.extension);
    const isAliasedExtensionForFile = (lang) => lang.extensionAliases?.find((aExt) => lang.fileName + "." + aExt === fileName);
    const isAliasedExtension = (lang) => lang.extensionAliases?.find((aExt) => aExt === ext[0]);
...
process.on("unhandledRejection", (err) => coc_nvim_1.workspace.createOutputChannel("Discord").appendLine(err));
